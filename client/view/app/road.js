...
module.exports = class Road {
  ...
  updateRoad(model) {
    ...
    for (i = 0; i < l; i += sizeNode) {
      ...
      if (roadType > 1) {
        ...
        positions[ctnP++] = this.pointsHeights[k] * this.tileHeight + 0.05;
  ...
  updateFoudation(model) {
    ...
    for (let i = 0, l = model.todo.length / 3; i < l; i++) {
      ...
      matrixWorld[14] = (model.todo[i * 3 + 1] + 0.5) * model.ground.tileSize;
      matrixWorld[13] = model.ground.getHeightTile(model.todo[i * 3], model.todo[i * 3 + 1]);
    }
    let toRemove = 0;
